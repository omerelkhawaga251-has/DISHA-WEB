...
import { POST_DESIGNS, YOUTUBE_THUMBNAILS, VIDEOS_AND_REELS, ProjectItem } from '@/data/portfolioData';
import { Maximize2, ArrowRight, Palette, Sparkles, ImageIcon, Flame, Youtube, Play } from 'lucide-react';
...
export default function DesignsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'posts' | 'thumbnails' | 'videos'>('posts');
    ...
    <div className="min-h-screen bg-[#050711] text-white font-sans selection:bg-orange-500 selection:text-white p-4 sm:p-8">
      ...
      <div className="max-w-7xl mx-auto text-center mb-12">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-orange-500/20 text-orange-400 text-xs font-mono font-bold uppercase tracking-wider mb-3 border border-orange-500/30">
          ...
          <span>CREATIVE PORTFOLIO HUB</span>
        </div>
        <h1 className="text-3xl sm:text-5xl font-black text-white tracking-tight">
          معرض المشاريع والتصاميم والفيديوهات
        </h1>
        <p className="text-stone-400 text-sm sm:text-base max-w-xl mx-auto mt-2 font-arabic" dir="rtl">
          تصفح كافة أعمال البوسترات التسويقية، الثمبنيلات، ومقاطع الفيديو والريلز مع إمكانية التكبير بدقة 4K والتشغيل الفوري
        </p>

        {/* 3 Tab Switchers */}
        <div className="inline-flex flex-wrap items-center justify-center gap-2 p-1.5 bg-stone-900/90 rounded-2xl border border-stone-800 mt-8 shadow-xl">
          <button
            onClick={() => setActiveTab('posts')}
            className={`px-5 sm:px-6 py-3 rounded-xl text-xs sm:text-sm font-bold transition-all flex items-center gap-2 ${
              activeTab === 'posts'
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        <AnimatePresence mode="popLayout">
          {currentList.map((project, idx) => {
            const isVideo = activeTab === 'videos' || !!project.videoUrl;
              ...
              >
                <TiltCard
                  ...
                  className={`h-full group bg-stone-950/80 backdrop-blur-xl rounded-3xl overflow-hidden border transition-all duration-500 cursor-pointer flex flex-col justify-between ${
                    isVideo
                      ? 'border-stone-800 hover:border-red-500/80 shadow-xl hover:shadow-[0_0_35px_rgba(239,68,68,0.25)]'
                      : isPost
                      ? 'border-stone-800 hover:border-orange-500 shadow-xl hover:shadow-[0_0_35px_rgba(249,115,22,0.25)]'
                      : 'border-stone-800 hover:border-amber-500 shadow-xl hover:shadow-[0_0_35px_rgba(245,158,11,0.25)]'
                  }`}
                >
                  <div className="relative aspect-[16/10] overflow-hidden bg-stone-900">
                    ...
                    <div className="absolute top-3 left-3 bg-gradient-to-r from-orange-600 to-amber-600 text-white text-[10px] font-black px-2.5 py-1 rounded-full flex items-center gap-1 shadow-lg">
                      {isVideo ? <Play className="w-2.5 h-2.5 fill-current" /> : <Flame className="w-3 h-3" />}
                      <span>{isVideo ? 'فيديو أصلي' : 'HIGH CTR'}</span>
                    </div>
                    <div className="absolute inset-0 bg-stone-950/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      ...
                      >
                        {isVideo ? <Play className="w-6 h-6 fill-current translate-x-0.5" /> : <Maximize2 className="w-6 h-6" />}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-6 flex flex-col justify-between flex-1 bg-stone-950/90 border-t border-stone-800">
                    <div>
                      <h3
                        className={`font-bold text-white text-base transition leading-snug ${
                          isVideo ? 'group-hover:text-red-400' : 'group-hover:text-orange-400'
                        }`}
                      >
                        {project.title}
                      </h3>
                    ...
                    <div
                      className={`pt-4 border-t border-stone-800 mt-4 flex items-center justify-between text-xs font-bold ${
                        isVideo ? 'text-red-400' : isPost ? 'text-orange-400' : 'text-amber-400'
                      }`}
                    >
                      <span className="flex items-center gap-1">
                        {isVideo ? (
                          <>
                            <span>مشاهدة الفيديو المباشر 🎬</span>
                      ...
                      <span className="text-stone-400 font-mono text-[11px]">
                        {isVideo ? 'Direct Play' : 'HD Artwork'}
                      </span>
                    </div>
                  </div>
                </TiltCard>
      ...
      {activeLightbox && (
        <ImageLightboxModal
          ...
          projects={activeTab === 'posts' ? POST_DESIGNS : YOUTUBE_THUMBNAILS}
